...
export class MatriculasService {
  ...
  async findDetailedByIdForUsuario(matriculaId: string, usuarioId: string): Promise<any> {
    ...
    const sponteAlunoId = matricula.sponteAlunoId;
    let sponteData: any = null;
    let sponteNeedsUpdate = false;
    let sponteUpdateFields: any = {};
    const sponteService = (this as any).sponteService || (global as any).sponteService;
    if (!sponteService && typeof require !== 'undefined') {
      try {
        const SponteService = require('../sponte/sponte.service').SponteService;
        (this as any).sponteService = new SponteService();
      } catch {}
    }
    if (sponteAlunoId && (this as any).sponteService) {
      try {

        const clienteCodigo = parseInt(process.env.SPONTE_CODIGO_CLIENTE || '0', 10);
        const clienteToken = process.env.SPONTE_TOKEN || '';
        const sponteXml = await (this as any).sponteService.getAlunos({
          nCodigoCliente: clienteCodigo,
          sToken: clienteToken,
          sParametrosBusca: `<IdAluno>${sponteAlunoId}</IdAluno>`
        });
        const parseField = (xml: string, tag: string) => {
          const m = xml.match(new RegExp(`<${tag}>([\\s\\S]*?)<\\/${tag}>`, 'i'));
          return m ? m[1].trim() : '';
        };
        sponteData = {
          nome: parseField(sponteXml, 'Nome'),
          genero: parseField(sponteXml, 'Sexo'),
          dataNascimento: parseField(sponteXml, 'DataNascimento'),
          cidade: parseField(sponteXml, 'Cidade'),
          bairro: parseField(sponteXml, 'Bairro'),
          cep: parseField(sponteXml, 'CEP'),
          endereco: parseField(sponteXml, 'Endereco'),
          numero: parseField(sponteXml, 'NumeroEndereco'),
          email: parseField(sponteXml, 'Email'),
          telefone: parseField(sponteXml, 'Telefone'),
          cpf: parseField(sponteXml, 'CPF'),
          rg: parseField(sponteXml, 'RG'),
          celular: parseField(sponteXml, 'Celular'),
          cidadeNatal: parseField(sponteXml, 'CidadeNatal'),
          nacionalidade: parseField(sponteXml, 'Nacionalidade'),
          cursoInteresse: parseField(sponteXml, 'CursoInteresse'),
        };
  const aluno = matricula.aluno as any;
        ...
        const compareField = (local: any, remote: any) => {
          if (sanitize(local) && sanitize(local) !== sanitize(remote)) return sanitize(local);
          return undefined;
        };
        sponteUpdateFields = {
          sNome: compareField(aluno.nome, sponteData.nome),
          sSexo: compareField(aluno.genero, sponteData.genero),
          dDataNascimento: compareField(aluno.dataNascimento, sponteData.dataNascimento),
          sCidade: compareField(aluno.cidade, sponteData.cidade),
          sBairro: compareField(aluno.bairro, sponteData.bairro),
          sCEP: compareField(aluno.cep, sponteData.cep),
          sEndereco: compareField(aluno.rua, sponteData.endereco),
          nNumeroEndereco: compareField(aluno.numero, sponteData.numero),
          sEmail: compareField(aluno.email, sponteData.email),
          sTelefone: compareField(aluno.telefone, sponteData.telefone),
          sCPF: compareField(aluno.cpf, sponteData.cpf),
          sRG: compareField(aluno.rg, sponteData.rg),
          sCelular: compareField(aluno.celular, sponteData.celular),
          sCidadeNatal: compareField(aluno.cidadeNatal, sponteData.cidadeNatal),
          sNacionalidade: compareField(aluno.nacionalidade, sponteData.nacionalidade),
          sCursoInteresse: compareField(aluno.cursoInteresse, sponteData.cursoInteresse),
        };
        Object.keys(sponteUpdateFields).forEach(k => {
          if (!sponteUpdateFields[k]) delete sponteUpdateFields[k];
        });
        if (Object.keys(sponteUpdateFields).length > 0) {
